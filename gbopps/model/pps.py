...
class PPS(nn.Module):
    def __init__(self, config):
        ...
        if self.neg_type == 'learnable_inverse_gaussian':
            ...
            # if self.mask_type == 'beta':
            #     self.fc_mask['l_inv_gauss']['beta'] = nn.Linear(config['hidden_dim'], self.num_props*self.num_neg_mask*2)
    ...
    def forward(self, frames_feat, frames_len, words_id, words_feat, words_len, weights, **kwargs):
        ...
        for mask in self.pos_mask_list:
            ...
            for i in range(self.num_props):
                ...
                gen = {
                    'gauss':     self.generate_gauss_mask,
                    'laplace':   self.generate_laplace_mask,
                    'cauchy':    self.generate_cauchy_mask,
                    'logistic':  self.generate_logistic_mask,
                    'tri':       self.generate_triangular_mask,
                    'epa':       self.generate_epanechnikov_mask,
                    'studentt':  self.generate_studentt_mask,
                    'rq':        self.generate_rq_mask,
                }
                try:
                    weight = gen[self.mask_type](props_len, mask_centers[mask][i], mask_widths[mask][i], num_mask=i+1)
                except KeyError:
        ...
        for mask in self.pos_mask_list:
            ...
            # prop_lefts[mask] = torch.clamp(prop_lefts[mask], min=0)
            # prop_rights[mask] = torch.clamp(prop_rights[mask], max=1)
    ...
    def generate_cauchy_mask(self, props_len, center, width, num_mask=1):
        weight = torch.linspace(0, 1, props_len, device=center.device)
        weight = weight.view(1, -1).expand(center.size(0), -1)
        mask_center = center.unsqueeze(-1)
        gamma = width.unsqueeze(-1).clamp(1e-2) / self.sigma_cauchy
        weight = 1/(1 + ((weight-mask_center)/gamma)**2)
        weight = weight/weight.max(dim=-1, keepdim=True)[0]
        return weight

    def generate_logistic_mask(self, props_len, center, width, num_mask=1):
        weight = torch.linspace(0, 1, props_len, device=center.device)
        weight = weight.view(1, -1).expand(center.size(0), -1)
        mask_center = center.unsqueeze(-1)
        scale = width.unsqueeze(-1).clamp(1e-2) / self.sigma_log
        z = (weight - mask_center)/scale
        sig = torch.sigmoid(z)
        weight = 4*sig*(1-sig)
        weight = weight/weight.max(dim=-1, keepdim=True)[0]
        return weight
    
    def generate_triangular_mask(self, props_len, center, width, num_mask=1):
        weight = torch.linspace(0, 1, props_len, device=center.device)
        weight = weight.view(1, -1).expand(center.size(0), -1)
        mask_center = center.unsqueeze(-1)
        halfwidth = width.unsqueeze(-1).clamp(1e-2) / self.sigma_tri
        weight = 1 - torch.abs((weight-mask_center)/halfwidth)
        weight = torch.clamp(weight, min=0.0)
        weight = weight/weight.max(dim=-1, keepdim=True)[0]
        return weight

    def generate_epanechnikov_mask(self, props_len, center, width, num_mask=1):
        weight = torch.linspace(0, 1, props_len, device=center.device)
        weight = weight.view(1, -1).expand(center.size(0), -1)
        mask_center = center.unsqueeze(-1)
        scale = width.unsqueeze(-1).clamp(1e-2) / abs(self.sigma_epa)
        dx = 1.0 / max(props_len - 1, 1)
        scale = torch.maximum(scale, torch.full_like(scale, dx))
        arg = (weight - mask_center)/scale
        weight = 1 - arg**2
        weight = torch.clamp(weight, min=0.0)
        weight = weight/weight.max(dim=-1, keepdim=True)[0]
        return weight

    def generate_studentt_mask(self, props_len, center, width, num_mask=1):
        weight = torch.linspace(0, 1, props_len, device=center.device)
        weight = weight.view(1, -1).expand(center.size(0), -1)
        mask_center = center.unsqueeze(-1)
        nu = float(self.nu_student)    # n
        d = 4.0 * (nu / (nu - 2.0))**0.5
        scale = width.unsqueeze(-1).clamp(1e-2) / d 
        arg = (weight - mask_center)
        weight = (1.0 + (arg**2) / (nu * scale**2)) ** (-(nu + 1.0) / 2.0)
        weight = weight / weight.max(dim=-1, keepdim=True)[0]
        return weight

    def generate_rq_mask(self, props_len, center, width, num_mask=1):
        weight = torch.linspace(0, 1, props_len, device=center.device)
        weight = weight.view(1, -1).expand(center.size(0), -1)
        mask_center = center.unsqueeze(-1)
        alpha = float(self.alpha_rq)   # a
        r = 2.0 * (2.0 * alpha * (2.0**(1.0/alpha) - 1.0))**0.5 / 0.5887050112577373
        scale = width.unsqueeze(-1).clamp(1e-2) / r
        arg = (weight - mask_center)
        weight = (1.0 + (arg**2) / (2.0 * alpha * scale**2)) ** (-alpha)
        weight = weight / weight.max(dim=-1, keepdim=True)[0]
        return weight
